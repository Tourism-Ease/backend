...
export const createBooking = asyncHandler(async (req, res, next) => {
  ...
  try {
    const { itemId, bookingType, paymentType, paymentMethod, counts, pickupCity, departureDate } = req.body;
    const userId = req.user._id;

    // Validate bookingType & paymentMethod
    ...
    // Determine the booking date
    let bookingDate;
    if (bookingType === 'Package') {
      bookingDate = item.departureDate; // use package model date
      if (!bookingDate) throw new APIError('Package departure date is missing', 500);
    } else {
      if (!departureDate) throw new APIError('Departure date is required for trips', 400);

      // Validate departureDate: tomorrow or later
      ...
      if (depDate < tomorrow) throw new APIError('Departure date must be tomorrow or later', 400);

      bookingDate = depDate;
    }

    // Calculate total price
    const priceData = bookingType === 'Trip'
    ...
    const bookingData = {
      ...
      paidAmount: paymentType === 'full' ? priceData.total : priceData.total * 0.5,
      remainingAmount: paymentType === 'full' ? 0 : priceData.total * 0.5,
      paymentStatus: paymentType === 'full' ? 'paid' : 'partial',
      bookingStatus: paymentMethod === 'cash' ? 'pending' : 'confirmed',
      bookingNumber: generateBookingNumber(),
      pickupCity: bookingType === 'Package' ? pickupCity || null : null // only for Package
    };
    ...
    const stripeMetadata = {
      ...
      dateTo: bookingDate.toISOString(),
      userId
    };

    if (bookingType === 'Package') stripeMetadata.pickupCity = pickupCity || null;

    // Decrement available seats
    if (bookingType === 'Package') {
      if (counts.adults + counts.children + counts.foreigners > item.availableSeats) {
        throw new APIError('Not enough available seats for this package', 400);
      }
      item.availableSeats -= (counts.adults + counts.children + counts.foreigners);
    ...
    if (paymentMethod === 'cash') {
      ...
      // Testing: 10 Seconds
      // bookingData.expiresAt = new Date(Date.now() + 10 * 1000); // 10 seconds
      // Testing: 2 minutes
        bookingData.expiresAt = new Date(Date.now() + 2 * 60 * 1000); // 2 minutes


      // bookingData.expiresAt = new Date(now.getTime() + 48 * 60 * 60 * 1000); // 48 hours
      ...
      await sendEmail(
        req.user.email,
        'Booking Created',
        `Your booking for ${item.title} is waiting for cash payment.`
      );
    ...
    const stripeAmount = Math.round(bookingData.remainingAmount * 100);
    const stripeSession = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          ...
          product_data: { name: `${bookingType} - ${item.title}` }
        },
        quantity: 1
      }],
      mode: 'payment',
...
export const bookingWebhookHandler = asyncHandler(async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    await sendEmail(user.email, 'Booking Confirmed', `Your booking is confirmed. Paid: ${paidAmount} EGP`);
  }
...
//     console.log(tempBooking.totalPrice);
//     console.log(tempBooking.remainingAmount);
